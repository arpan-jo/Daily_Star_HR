...
const MarketVisitApprovalDetails = ({route}: props) => {
  ...
  const approveOrReject = async (
    ...
    if (res?.StatusCode === 500) {
      toaster.show({message: res?.message || res, type: 'error'});
    }
  };
    ...
    async isMounted => {
      ...
      const api_params = {
        ...
        data: {
          ...
          intId: attDetails?.application?.intEmployeeId,
          ApplicationDate: attDetails?.application?.dteAttendanceDate,
        },
      };
      const attDncePunch = await httpRequest(api_params, setIsLoading);
              ...
              <View>
                {location ? (
                  <MapView
                    ...
                    provider="google"
                    initialRegion={{
                      latitude: polylineLatLng?.[0]?.latitude
                        ? polylineLatLng?.[0]?.latitude
                        : lat,
                      longitude: polylineLatLng?.[0]?.longitude
                  ...
                  <View style={[styles.rowSpaceBetween, styles.marginTop]}>
                    ...
                    <View style={styles.status}>
                      <Text style={styles.timeTxt}>
                        {attDetails?.waitingStage ||
                          attDetails?.WaitingStage ||
                          attDetails?.currentStage}
                      </Text>
                    </View>
                  </View>
                </View>
...
export default MarketVisitApprovalDetails;
